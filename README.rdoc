...
(e.g. +config/initializer/couch_model.rb+).
...
This example uses an sub-directory of +app/models+ to search for the design documents. It also defined a constant
named +DATABASE+ that is initialized with the right database setup for the each environment. This constant can then be
